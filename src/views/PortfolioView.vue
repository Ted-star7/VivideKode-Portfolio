  ...
  <div>
      ...
      <div class="container page-hero__inner">
        <div ref="heroText">
          ...
          <p
            ...
            "
          >
            A curated selection of our most impactful client engagements — each
            representing our commitment to engineering excellence and measurable
            business outcomes.
          </p>
        </div>
      </div>
    ...
    <section class="section section-lg">
      <div class="container">
        <!-- Filter pills -->
        <div
          v-if="store.projects.length > 0 && !store.isLoading"
          class="filter-bar"
          ref="filterBar"
          role="group"
        ...
        <!-- Loading State -->
        <div v-if="store.isLoading" class="loading-state" role="status">
        ...
        <div v-else-if="store.error" class="error-state" role="alert">
          ...
          <button @click="retryLoad" class="btn-outline-navy btn">
            Try Again
          </button>
        </div>
        ...
        <!-- Project grid -->
        <TransitionGroup 
          v-else-if="!store.isLoading && !store.error && store.projects.length > 0" 
          name="proj-list" 
          tag="div" 
          class="proj-grid"
        >
          ...
          >
            <div
              class="proj-card__thumb"
              :style="p.imageUrl ? {
                backgroundImage: `url(${p.imageUrl})`,
                backgroundSize: 'cover',
                backgroundPosition: 'center',
              } : {
                background: `linear-gradient(135deg, #1B3A5C 0%, #0d2d4a 100%)`,
              }"
            >
              <div class="proj-card__overlay" aria-hidden="true" />
              <div class="proj-card__meta">
                <span
                  class="proj-card__cat"
                  :style="{
                    color: p.accent,
                ...
                <span
                  v-if="p.status"
                  class="proj-card__status vk-badge"
                  style="
            ...
            <div class="proj-card__body">
              ...
              <div class="proj-card__tags">
                <span
                  ...
                  >{{ t }}</span
                >
                <span class="vk-badge" v-if="p.tags.length > 3"
        ...
        <div
          v-if="!store.isLoading && !store.error && store.projects.length > 0 && store.filtered.length === 0"
          class="empty-state"
          role="status"
        >
    ...
    <Transition name="modal">
      <div
        ...
        role="dialog"
        :aria-labelledby="`modal-title-${activeProject.id}`"
        aria-modal="true"
      >
        <div class="modal-box modal-proj">
          <div
            class="modal-proj__thumb"
            :style="activeProject.imageUrl ? {
              backgroundImage: `url(${activeProject.imageUrl})`,
              backgroundSize: 'cover',
              backgroundPosition: 'center',
            } : {
              background: `linear-gradient(135deg, #1B3A5C 0%, #0d2d4a 100%)`,
            ...
            <div class="modal-proj__thumb-content">
              <span
                ...
                :style="{
                  ...
                  background: activeProject.accent + '22',
                  border: `1px solid ${activeProject.accent}44`,
                }"
              >
                {{ activeProject.category }}
              </span>
              <h2
                class="modal-proj__title"
                :id="`modal-title-${activeProject.id}`"
              >
                {{ activeProject.title }}
              </h2>
              <p class="modal-proj__client">
          ...
          <div class="modal-proj__body">
            ...
            <div class="modal-proj__results">
              <h3 class="modal-proj__section-title">Key Details</h3>
              <ul class="results-list" role="list">
                <li
                  v-for="r in activeProject.results"
                  :key="r"
                  class="result-item"
                >
            ...
            <div class="modal-proj__actions">
              <a
                v-if="activeProject.projectLink"
    ...
    <section class="cta-strip">
      <div class="container cta-strip__inner">
        <h2 class="heading-lg" style="color: #fff">
          Ready to become<br /><em
            style="color: var(--gold); font-style: normal"
            >our next success story?</em
          >
        </h2>
...
useSeo({
  ...
  description:
    "Explore VividKode Creations portfolio — web development, AI, data engineering, and custom software projects with real, measurable results.",
  canonical: "/portfolio",
});
...
async function retryLoad() {
  ...
  if (store.projects.length > 0 && filterBar.value) {
...
onMounted(async () => {
  ...
  if (filterBar.value && store.projects.length > 0 && !store.error) {
    scrollReveal(filterBar.value);
  }
});
</script>
...
.modal-proj__close:hover {
  background: rgba(0, 0, 0, 0.65);
}
.modal-proj__thumb-content {
  position: relative;
  z-index: 1;
}
.modal-proj__title {
...
/* TransitionGroup */
.proj-list-move,
.proj-list-enter-active,
.proj-list-leave-active {
  transition: all 0.38s cubic-bezier(0.25, 0.46, 0.45, 0.94);
}
.proj-list-enter-from,
.proj-list-leave-to {
  opacity: 0;
